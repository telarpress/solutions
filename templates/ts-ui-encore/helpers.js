...
/**
 * Checks if encore is installed on the system.
 */
export async function checkEncoreInstallation() {
  return new Promise((resolve, reject) => {
    exec('encore -v', (error, stdout, stderr) => {
      if (error) {
        reject('Encore is not installed.');
      }
      resolve('Encore is installed:' + stdout);
    });
  });
}
